...
ibm_font_ex_small = ImageFont.truetype(ibm_font_path, size=30)

text_color_white = (255, 255, 255)
text_color_green = (46, 189, 133)
text_color_red = (246, 70, 93)

text_color_binance = (243, 186, 47)
text_color_bybit = (247,166,0)
...
def draw_symbol_to_entry_price(exchange, draw, symbol, amount, entry_price, realized=False):
    # exchange
    text_position = (110, 100)
    if exchange.lower() == 'binance':
        draw.text(text_position, 'Binance', font=ibm_font_ex_small, fill=text_color_binance)
    elif exchange.lower() == 'bybit':
        draw.text(text_position, 'Bybit', font=ibm_font_ex_small, fill=text_color_bybit)
...
def create_new_position_image(exchange:str, symbol: str, amount: str, entry_price: str) -> io.BytesIO:
    # get image file
    image = Image.open('resource/image/background_new_position.png')

    draw = ImageDraw.Draw(image)
    draw = draw_symbol_to_entry_price(exchange, draw, symbol, amount, entry_price)
...
def create_position_image(exchange: str, symbol: str, amount: str, entry_price: str, mark_price: str, profit: str,
                          realized=False) -> io.BytesIO:
    # select image file
    if float(profit) > 0:
        image_path = f'resource/image/background_profit_{random.randint(1, 3)}.png'
    else:
        image_path = f'resource/image/background_loss_{random.randint(1, 6)}.png'

    image = Image.open(image_path)

    # todo draw exchange symbol
    draw = ImageDraw.Draw(image)
    draw = draw_symbol_to_entry_price(exchange, draw, symbol, amount, entry_price, realized=realized)

    # mark price
    text_position = (110, 330)
